...
def pyxLen(a):
    assert(len(a) == 2)
    return math.sqrt(pyx.unit.topt(a[0])**2 + pyx.unit.topt(a[1])**2)


def pyxSplit(a, parameter=0.5):
    assert len(a) == 2
    assert isinstance(a[0], pyx.path.moveto_pt)
    assert isinstance(a[1], pyx.path.curveto_pt)
    p0 = (a[0].x_pt, a[0].y_pt)
    p1 = (a[1].x1_pt, a[1].y1_pt)
    p2 = (a[1].x2_pt, a[1].y2_pt)
    p3 = (a[1].x3_pt, a[1].y3_pt)
    newcurves = pyx.path.path()
    newcurves.append(pyx.path.moveto(p0[0], p0[1]))
    ...
    newcurves.append(pyx.path.curveto(p4[0], p4[1],
                                      p7[0], p7[1],
                                      p9[0], p9[1]))
    newcurves.append(pyx.path.curveto(p8[0], p8[1],
                                      p6[0], p6[1],
                                      p3[0], p3[1]))
    return newcurves
